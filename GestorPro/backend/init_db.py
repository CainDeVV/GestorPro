...
def reset_all_data_force():
    """
    Função para resetar todos os dados do banco FORÇADAMENTE.
    Remove todos os registros ignorando regras de FK e transações.
    """
    ...
    try:
        print("🔄 Iniciando reset FORÇADO de todos os dados...")
        
        # Primeira conexão: limpar dados
        conn = sqlite3.connect(DB_NAME)
        cursor = conn.cursor()
        
        # Desabilita TUDO que pode interferir
        ...
        # Commit e fecha primeira conexão
        conn.commit()
        conn.close()
        
        # Segunda conexão: VACUUM (fora de transação)
        print("🧹 Otimizando banco de dados...")
        conn2 = sqlite3.connect(DB_NAME)
        conn2.execute("VACUUM;")
        conn2.close()
        
        # Terceira conexão: reabilitar configurações
        conn3 = sqlite3.connect(DB_NAME)
        cursor3 = conn3.cursor()
        cursor3.execute("PRAGMA foreign_keys = ON;")
        cursor3.execute("PRAGMA journal_mode = WAL;")
        cursor3.execute("PRAGMA synchronous = NORMAL;")
        conn3.close()
        
        print("✅ Reset FORÇADO realizado com sucesso!")
        print("🎉 Todos os dados foram removidos do banco.")
        return True
        
    except Exception as e:
        print(f"[ERRO] Falha no reset forçado: {e}")
        return False
